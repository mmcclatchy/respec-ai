...
2. Updates all mcp__specter__* calls to include project_path parameter
...
def update_command_template(file_path: Path) -> tuple[bool, str]:
    ...
    # Update MCP calls - pattern: mcp__specter__tool_name(params)
    # We need to add project_path=PROJECT_PATH as first parameter

    # Pattern 1: Simple calls with just one param
    content = re.sub(r'mcp__specter__(\w+)\(([^,)]+)\)', r'mcp__specter__\1(project_path=PROJECT_PATH, \2)', content)

    # Pattern 2: Calls with multiple params
    content = re.sub(
        r'mcp__specter__(\w+)\(([^)]+, [^)]+)\)', r'mcp__specter__\1(project_path=PROJECT_PATH, \2)', content
    )

    # Pattern 3: Calls with no params
    content = re.sub(r'mcp__specter__(\w+)\(\)', r'mcp__specter__\1(project_path=PROJECT_PATH)', content)
...
def update_agent_template(file_path: Path) -> tuple[bool, str]:
    ...
    # Add project_path documentation to INPUTS section if mcp__specter__ calls exist
    if 'mcp__specter__' in content and 'project_path' not in content:
        # Find INPUTS section
        pattern = r'(INPUTS:.*?\n)'
    ...
    content = re.sub(r'mcp__specter__(\w+)\(([^,)]+)\)', r'mcp__specter__\1(project_path, \2)', content)

    content = re.sub(r'mcp__specter__(\w+)\(([^)]+, [^)]+)\)', r'mcp__specter__\1(project_path, \2)', content)

    content = re.sub(r'mcp__specter__(\w+)\(\)', r'mcp__specter__\1(project_path)', content)
