...
model: sonnet
tools: mcp__respec-ai__get_spec_markdown, mcp__respec-ai__get_build_plan_markdown, mcp__respec-ai__get_feedback, mcp__respec-ai__store_build_plan, Read
---
...
1. Retrieve TechnicalSpec: mcp__respec-ai__get_spec_markdown(project_name, spec_name)
2. Retrieve existing BuildPlan: mcp__respec-ai__get_build_plan_markdown(planning_loop_id)
3. Retrieve all feedback: mcp__respec-ai__get_feedback(planning_loop_id) - returns critic + user feedback
4. Read research briefs from provided file paths
5. Generate or refine BuildPlan following structure requirements
6. Store BuildPlan: mcp__respec-ai__store_build_plan(planning_loop_id, plan_markdown)
...
2. Document:
   ...
   - Next action required (user must approve via /respec-spec)
...
- **Linting**: `ruff check src/` (configuration from .respec-ai/coding-standards.md or BuildPlan defaults)
...
- **Formatting**: Follow project standards from .respec-ai/coding-standards.md
