...
def generate_infrastructure_reviewer_template(tools: InfrastructureReviewerAgentTools) -> str:
    ...
    output_contract = render_reviewer_output_contract('infrastructure-reviewer', tools.store_reviewer_result)
...
6. Extract runtime platform, deployment mechanism, secret source, CI commands, and environment wiring from stack config, Phase, the implementation plan, and workflow guidance.
7. Extract `.best-practices/` paths from Phase `### Research Requirements` and its nested `- Applied:` annotations; read docs relevant to infrastructure behavior under review.
