...
class TestMaterializeSkeletonsCommand:
    def test_writes_skeletons_and_tests_under_the_current_working_directory(
        ...
        index_file = _write(tmp_path / 'index.md', '- `src/kb/client.py` :: Client.query(cypher: str) -> list[str]\n')
        test_file = _write(tmp_path / 'tests.md', '- `tests/unit/test_client.py::test_query_returns_a_list`\n')
