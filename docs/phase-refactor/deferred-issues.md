...
| Phase 6 — Task removal (docs pass) | `docs/ARCHITECTURE.md` (directory-tree section, `respec-code-reviewer.md` / `respec-coding-standards-reviewer.md` entries) | These generated-agent filenames don't match the current template's naming (`code_quality_reviewer.py` → presumably `respec-code-quality-reviewer.md`), and the doc's top-level counts ("6 command templates", "20 specialized agent templates" after my edit) still don't reconcile with `commit_command.py`/`standards_command.py` or `review-consolidator`/`research-synthesis-orchestrator`, which are referenced elsewhere in the same doc but have no corresponding `.py` template file under `src/platform/templates/`. | Pre-existing drift unrelated to Task removal — fixing it means reconciling the whole template inventory/naming scheme, out of scope for a Task-reference cleanup pass. |
| Phase 6 — Task removal (docs pass) | `docs/TEMPLATE_AUDIT.md:3` | Top-line summary "Comprehensive audit of all 29 templates (7 commands + 22 agents)" doesn't match either the pre-Phase-6 count (9 commands / 20 agents, per the Phase 6 task brief) or the current count (8 commands / 18 agents). It's a mismatch independent of Task removal — looks like stale drift from an earlier, smaller template set. | Not caused by Task removal; recomputing the doc's original historical scope is outside a removal-and-correction pass. |
